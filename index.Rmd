...
```{r echo=FALSE, fig.width=15}
 sim <- read.table("./ch0_sim.dat")
 exp <- read.table("./ch1_expt.dat")
plot(sim, type = "s", ylim=c(-1000,100), xlim=c(0,4100), xlab = ' ', ylab=' ', col='red')
lines(exp, type = "s", col='blue')
...
```{r echo=FALSE}
library(ggplot2)
data1 <- read.table("./720.dat", sep = " ")
data1 <- data1[,c(1,2)]
colnames(data1) <- c("bin", "value")
plot(data1$value, type = "s", ylim=c(3,1050), xlim=c(38,1000), xlab = 'Energy', ylab='Events')

```

***=right
```{r echo=FALSE}
plot(data1$value, type = "s", ylim=c(3,1050), xlim=c(735,755), xlab = 'Energy', ylab='Events')
```
